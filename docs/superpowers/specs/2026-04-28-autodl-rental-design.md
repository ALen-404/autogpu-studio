...
本方案基于 AutoGPU Studio 的非商业开源定位调整。当前项目继承了 `waoowaoo` 的 Next.js、Prisma、MySQL、Redis、BullMQ、对象存储、任务状态和多模型配置中心等基础能力。由于上游许可证为 `CC BY-NC-SA 4.0`，本项目不做商业租机平台，不收取租机费用，不做 AutoDL 算力转售，也不做站内余额冻结和加价结算。
...
- 不收费：平台不向用户收取 GPU 租机费、服务费、抽成或差价。
- 不转售：平台不使用统一 AutoDL 账号替用户代租机器。
...
- 用户可以通过 AutoDL 官方入口自行租机，然后把远程 Worker 地址填回平台。
...
3. 用户在 AutoDL 官方页面完成租机、付款、镜像选择和实例启动。
4. 用户在实例中启动远程 Worker。
