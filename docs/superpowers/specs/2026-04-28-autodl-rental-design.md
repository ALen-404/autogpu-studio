...
本方案基于 AutoGPU Studio 的派生架构扩展。当前项目继承了 `waoowaoo` 的 Next.js、Prisma、MySQL、Redis、BullMQ、对象存储、任务状态、余额冻结和多模型配置中心等基础能力。目标是在平台内提供按小时租用 GPU 机器的能力，用户不接触 AutoDL 控制台，只在平台内使用视频、图片和配音模型。
